...
> First-time AI-readiness setup for this repository

## Purpose

This workflow generates accurate, compact "current state" documentation that makes
...
9. Recommended next steps after bootstrap:
   a. Run `/garden-audit` to verify documentation accuracy
...
Present a comprehensive discovery summary to the user. Include:
- All findings from sections 1.1-1.7
- Flag any uncertain discoveries for review
- Ask: "Is there important context that lives outside this repo (Slack decisions, wiki pages, design docs) that should be captured in `docs/`?"
- Ask about pre-existing AI config files (if any): merge, replace, or skip?
- Ask: "Does your team use JetBrains IDEs? (This determines if we generate .junie/guidelines.md)"
- Request explicit confirmation to proceed to Phase 2
...
| `.junie/guidelines.md` | `# Junie Guidelines\n\nFollow all instructions in the root AGENTS.md file as the primary context for this repository.` | **Only generate if user confirmed JetBrains usage in Phase 1** |
...
After generating all files, display a coverage tracker showing what was created:

```markdown
## 🌱 Bootstrap Complete! Garden Health Status

| Category | File | Status |
|----------|------|--------|
| Source of Truth | AGENTS.md | ✅ Created ({line_count} lines) |
| Claude Code | CLAUDE.md | ✅ Created |
| GitHub Copilot | .github/copilot-instructions.md | ✅ Created |
| Cursor | .cursor/rules/agents.mdc | ✅ Created |
...
All wrapper files reference AGENTS.md as the source of truth. Any AI tool can now
understand your project's context.

### Recommended Next Steps

1. **Verify accuracy:** Run `/garden-audit` to check that docs match actual code
2. **Add guardrails:** Run `/garden-extend` to add coding principles and conventions
3. **Fetch references:** Run `/garden-references` to get llms.txt for your dependencies
4. **Monthly maintenance:** Set a reminder to run `/garden-maintain` for ongoing health

### Commit Your Changes

```bash
git add .
git commit -m "chore: bootstrap AI agent configuration"
```

🌱 Happy coding with your AI agents! Run `/garden-agent-gardener` anytime for maintenance.
```

## Critical Rules

- Phase 1: MUST get user confirmation before proceeding to Phase 2
- AGENTS.md: MUST be under 150 lines (strict)
- Wrappers: MUST reference AGENTS.md, not duplicate content
- .junie/guidelines.md: ONLY create if user confirms JetBrains usage
- No fabricated content: Stubs are better than invented documentation
- Verification: MUST show coverage tracker at the end
- Garden system references: Use correct command names (/garden-agent-gardener, /garden-maintain)
